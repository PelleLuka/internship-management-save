...
const VALIDATION_ERRORS = {
  ...
  INVALID_DATE_FORMAT: [
    400,
    'Dates must be in YYYY-MM-DD format and must be valid dates',
  ],
...
export const addActivityToInternship = async (req, res) => {
  try {
    ...
    const activity = await internshipService.addActivityToInternship(
      internshipId,
      activityId,
    );
    ...
    if (err.message === 'NOT_FOUND') {
      return res
        .status(404)
        .json({ error: 'Internship or Activity not found' });
    }
...
export const removeActivityFromInternship = async (req, res) => {
  try {
    ...
    await internshipService.removeActivityFromInternship(
      internshipId,
      activityId,
    );
    res.status(204).send();
