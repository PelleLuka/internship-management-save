...
import { Search, Plus, Pencil, Trash2, Activity as ActivityIcon, X } from "lucide-vue-next";
...
const saveCategories = async (activity) => {
  ...
  await updateActivity(activity.id, { categoryIds: [...existing, ...newIds] });
  closeCategoryMenu();
  await loadActivities();
};

const removeCategoryFromActivity = async (activity, catId) => {
  const remaining = (activity.categories ?? []).filter(c => c.id !== catId).map(c => c.id);
            ...
            <div class="relative mb-3">
              ...
              >
                <div class="flex justify-between items-center mb-3">
                  ...
                  <button @click.stop="closeCategoryMenu" class="text-slate-400 hover:text-slate-600">
                    <X class="w-4 h-4" />
                  </button>
                </div>
                ...
                <div class="flex justify-end gap-2 pt-2 border-t border-slate-100">
                  <AppButton variant="outline" size="sm" @click.stop="closeCategoryMenu">Annuler</AppButton>
                  <AppButton
                    size="sm"
