...
<template>
  <div class="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition-all duration-200 p-5 flex flex-col gap-3">
    ...
    <div>
      ...
      <p v-if="category.description" class="text-sm text-slate-500 mt-1 line-clamp-2">{{ category.description }}</p>
    </div>

    <div class="mt-auto pt-2 border-t border-slate-100">
      <span class="text-xs text-slate-400">
        {{ category.activityCount }} atelier{{ category.activityCount !== 1 ? 's' : '' }} lié{{ category.activityCount !== 1 ? 's' : '' }}
      </span>
    </div>
  </div>
</template>
