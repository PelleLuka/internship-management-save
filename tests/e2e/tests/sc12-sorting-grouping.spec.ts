...
test.describe('SC12 - Tri et Groupement', () => {
  ...
  test('Vérifier le tri de la liste', async ({ page }) => {
    ...
    const titles = page
      ...
      .filter({ hasText: `SortTest-${uniqueId}` });
    ...
    await page.getByPlaceholder('Rechercher...').fill(`SortTest-${uniqueId}`);
    await page
      .getByTestId(`delete-${userA.firstName}-${userA.lastName}`)
      .click()
      .catch(() =>
        page
          .getByLabel(`Supprimer ${userA.firstName} ${userA.lastName}`)
          .click(),
      );
    await page
      .getByTestId(`delete-${userZ.firstName}-${userZ.lastName}`)
      .click()
      .catch(() =>
        page
          .getByLabel(`Supprimer ${userZ.firstName} ${userZ.lastName}`)
          .click(),
      );
  });
});
