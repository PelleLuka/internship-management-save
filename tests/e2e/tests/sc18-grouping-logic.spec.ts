...
test.describe('SC18 - Logique de Groupement', () => {
  ...
  test('Vérifier le groupement par Année et Mois', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: '2025', exact: true }),
    ).toBeVisible();
    ...
    await expect(
      page
        .locator('#year-2024')
        .getByRole('heading', { name: /Mai/i, level: 3 })
        .first(),
    ).toBeVisible();
    ...
    await expect(
      page
        .locator('#year-2025')
        .getByRole('heading', { name: /Janvier/i, level: 3 })
        .first(),
    ).toBeVisible();
    ...
    // Suppression propre
    await page
      .getByTestId(`delete-${user2024.firstName}-${user2024.lastName}`)
      .click()
      .catch(() => {});
    await page
      .getByTestId(`delete-${user2025.firstName}-${user2025.lastName}`)
      .click()
      .catch(() => {});
  });
});
