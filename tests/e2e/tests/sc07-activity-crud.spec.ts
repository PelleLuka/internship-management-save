...
test.describe('SC07 - CRUD Activités', () => {
  ...
  test("Gérer le cycle de vie d'une activité", async ({ page }) => {
    ...
    try {
      ...
      console.log(
        'Heading "Activités" not found, continuing (likely mobile view)',
      );
    }
    ...
    console.log(
      `Searching for button testid: "edit-activity-${activityTitle}"`,
    );
    await expect(editButton).toBeVisible();
    // Scroll to center to avoid Sticky Header overlap (Critical for Mobile/Firefox)
    await editButton.evaluate((node: HTMLElement) =>
    ...
    await page.reload();
    await expect(page.getByText(activityTitleUpdated)).toBeVisible({
      timeout: 15000,
    });
    ...
    await page.reload();
    await expect(page.getByText(activityTitleUpdated)).toBeHidden({
      timeout: 10000,
    });
  });
});
