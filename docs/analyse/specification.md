...
Des ébauches conceptuelles ont été réalisées pour visualiser l'architecture et les interactions au sein de l'application. Chaque concept décrit est considéré comme mis en place et valide pour la suite.
...
- **Intégrité référentielle** :
  - **Suppression d'un stagiaire** : Si un stagiaire est supprimé, toutes ses associations avec des activités sont automatiquement supprimées dans `internship_activities` (Cascade).
