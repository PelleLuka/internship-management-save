# CA TIC WorkXP Admin — Rapport TPI : Plan de rédaction

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Produire `docs/rapport-tpi/rapport-tpi-workxpadmin.md` — contenu complet du rapport TPI WorkXP Admin, prêt à copier dans le gabarit Word CA TIC.

**Architecture:** Document Markdown unique avec une section par chapitre du gabarit HEIA-FR v1.4. Les sections `[RÉDIGÉ]` sont entièrement rédigées. Les sections `[GUIDÉ]` contiennent un plan structuré et des phrases d'accroche. Les sections `[CANDIDAT]` contiennent uniquement des placeholders explicites. Style calqué sur eParking 2024 et eRecrutement 2025 (même CA TIC).

**Tech Stack:** Markdown, gabarit Word HEIA-FR v1.4, références: `docs/superpowers/specs/2026-05-04-rapport-tpi-design.md`, CdC: `TPI 2026 Pellegrinelli Luka (4).pdf`, schéma DB: `database/schema-v2.sql`

---

## Fichiers

- **Créer :** `docs/rapport-tpi/rapport-tpi-workxpadmin.md` — document principal
- **Référence :** `docs/superpowers/specs/2026-05-04-rapport-tpi-design.md` — spec complète
- **Référence :** `database/schema-v2.sql` — schéma DB pour section 5.3
- **Référence :** `backend/routes/internshipRoutes.js`, `backend/routes/activityRoutes.js`, `backend/routes/categoryRoutes.js`, `backend/routes/certificateRoutes.js` — API endpoints pour section 6
- **Référence :** `tests/e2e/tests/` — scénarios Playwright pour section 7
- **Référence :** `tests/api/test_internship_management.postman_collection.json` — tests API pour section 7

---

## Conventions de style (à respecter dans tout le document)

- Titres de section : `## 1. Titre`, sous-sections : `### 1.1 Titre`, `#### 1.1.1 Titre`
- Tableaux use case : colonnes **Quoi / Pourquoi / Comment / Contrainte / Remarque** (omis si vide), header bleu simulé en Markdown avec `**Nom du cas d'utilisation**`
- Ton : professionnel, français formel, phrases complètes justifiées à gauche
- Placeholder candidat : `> ⚠️ **À COMPLÉTER PAR LE CANDIDAT** : [instruction précise]`
- Références aux figures : `*Figure N — Description*` sous chaque schéma/diagramme (les diagrammes sont décrits textuellement car en Markdown — le candidat insère les images dans Word)

---

## Task 1 : Scaffolding + Résumé du rapport

**Files:**
- Create: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Créer le fichier avec le squelette complet**

Créer `docs/rapport-tpi/rapport-tpi-workxpadmin.md` avec tous les titres de sections (1 à 15) vides. Cela permet de vérifier la structure complète avant de remplir.
...
- [ ] **Step 2 : Rédiger la section 1 — Résumé du rapport**

Remplir la section `## 1. Résumé du rapport` avec le contenu suivant (3 blocs, style eParking/eRecrutement) :
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): scaffolding + section 1 résumé du rapport"
```

---

## Task 2 : Préambule (sections 2.1, 2.4, 2.5)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 2.1 Introduction**
...
- [ ] **Step 2 : Insérer les placeholders candidat pour 2.2 et 2.3**
...
- [ ] **Step 3 : Rédiger 2.4 Déroulement du projet**
...
- [ ] **Step 4 : Rédiger 2.5 Déclarations**
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 2 préambule"
```

---

## Task 3 : Cahier des charges (section 3)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`
- Read: `TPI 2026 Pellegrinelli Luka (4).pdf` (pages 3–6 déjà lues en session)
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 3 cahier des charges"
```

---

## Task 4 : Analyse — État initial, désiré, public cible (4.1–4.3)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 4.1, 4.2, 4.3**
...
Actuellement, le suivi administratif de ces stages est assuré manuellement via Microsoft
OneNote. Pour chaque période de stage, une note recense la liste des participants ainsi
que les ateliers qu'ils ont choisi de réaliser. Ce système présente plusieurs limitations :

- Aucune centralisation structurée : les données sont réparties dans de multiples notes
...
> ⚠️ **À COMPLÉTER PAR LE CANDIDAT** : Insérer la capture d'écran OneNote fournie dans le CdC
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 4.1–4.3 analyse état initial/désiré/public"
```

---

## Task 5 : Analyse — Besoins (4.4)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 4.4.1 Cas d'utilisation**
...
- [ ] **Step 2 : Rédiger 4.4.2 Description (tableaux Quoi/Pourquoi/Comment)**

Écrire les tableaux pour chaque cas d'utilisation. Format : titre en **gras**, tableau à 2 colonnes (libellé | valeur). Exemple pour le premier :
...
| **Contrainte** | Impossible si le stage est lié à au moins un stage (HTTP 409 retourné) |
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 4.4 besoins et cas d'utilisation"
```

---

## Task 6 : Analyse — Variantes et Technologies (4.5–4.6)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 4.5 Fonctionnement — Variantes**
...
- [ ] **Step 2 : Rédiger 4.6 Technologies utilisées**
...
| **Postman / Newman** | Tests d'intégration API | Validation du contrat d'interface de l'API REST, exécution en ligne de commande via Newman. |
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 4.5–4.6 variantes et technologies"
```

---

## Task 7 : Conception — Architecture + Séquences + Base de données (5.1–5.3)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`
- Read: `database/schema-v2.sql`

- [ ] **Step 1 : Lire le schéma DB**

```bash
cat database/schema-v2.sql
```

- [ ] **Step 2 : Rédiger 5.1 Schéma d'architecture**
...
- [ ] **Step 3 : Rédiger 5.2 Diagrammes de séquences**
...
- [ ] **Step 4 : Rédiger 5.3 Base de données**

Lire `database/schema-v2.sql` et produire la description complète :
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 5.1–5.3 architecture, séquences, base de données"
```

---

## Task 8 : Conception — Mockups + Stratégie de tests (5.4–5.5)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 5.4 Mockups**
...
- [ ] **Step 2 : Rédiger 5.5 Stratégie de tests**
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 5.4–5.5 mockups et stratégie de tests"
```

---

## Task 9 : Réalisation — Structure, Biome, Design (6.1–6.3)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`
- Read: `package.json` (racine), `biome.json`

- [ ] **Step 1 : Lire les fichiers de configuration**
...
- [ ] **Step 2 : Rédiger 6.1 Structure du projet**
...
- [ ] **Step 3 : Rédiger 6.2 Linter et code formatter**

Lire `biome.json` puis écrire :
...
- [ ] **Step 4 : Rédiger 6.3 Design et interface**
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 6.1–6.3 structure, biome, design"
```

---

## Task 10 : Réalisation — Fonctionnalités métier (6.4–6.6)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 6.4 Gestion des stagiaires**
...
- [ ] **Step 2 : Rédiger 6.5 Gestion des ateliers**
...
- [ ] **Step 3 : Rédiger 6.6 Gestion des catégories**
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 6.4–6.6 fonctionnalités métier"
```

---

## Task 11 : Réalisation — Certificat, DB, Erreurs (6.7–6.9)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger 6.7 Génération du certificat**
...
- [ ] **Step 2 : Rédiger 6.8 Base de données**
...
- [ ] **Step 3 : Rédiger 6.9 Traitement des erreurs**
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): section 6.7–6.9 certificat, base de données, erreurs"
```

---

## Task 12 : Tests, Problèmes, Améliorations, Glossaire (sections 7–9 + 13)

**Files:**
- Modify: `docs/rapport-tpi/rapport-tpi-workxpadmin.md`

- [ ] **Step 1 : Rédiger section 7 — Tests de fonctionnement**
...
> « Statut » (✅ / ❌) après exécution des tests lors du TPI. Ajouter la signature
> du chef de projet.
```

- [ ] **Step 2 : Rédiger section 8 — Problèmes rencontrés (guidé)**
...
>
> **Points à documenter (pistes — compléter avec ton vécu) :**
>
...
> Contexte : Les boutons d'action des cartes (édition, suppression, certificat) sont
> masqués par pointer-events: none en dehors du survol. Playwright ne pouvait pas
> les cliquer normalement.
...
>
> **Ajouter ici les autres problèmes rencontrés** : bugs inattendus, difficultés de
> configuration Docker, problèmes de CORS, comportements inattendus de MariaDB, etc.
```

- [ ] **Step 3 : Rédiger section 9 — Amélioration et évolution**
...
mais sorties du périmètre du TPI :
...
- [ ] **Step 4 : Rédiger section 13 — Glossaire**
...
- [ ] **Step 5 : Rédiger les sections 10–12 et 14–15 (placeholders candidat)**
...
> - Bilan personnel : compétences acquises, apprentissages clés
...
clairement référencés.

> ⚠️ **À SIGNER PAR LE CANDIDAT** — Date : _____ / _____ / 2026
...
> - Listings de code des composants principaux (si demandé)
> - Collection Postman (export JSON)

## 15. Table des illustrations

> ⚠️ **À GÉNÉRER AUTOMATIQUEMENT DANS WORD** : Insertion → Table des figures
```
...
```bash
git add docs/rapport-tpi/rapport-tpi-workxpadmin.md
git commit -m "docs(tpi): sections 7-13 tests, problèmes, améliorations, glossaire, clôture"
```
...
**1. Couverture spec :**
- ✅ Résumé (3 blocs) → Task 1
- ✅ Préambule complet → Task 2
- ✅ Cahier des charges avec tableau exigences → Task 3
- ✅ État initial/désiré/public cible → Task 4
- ✅ Cas d'utilisation + tableaux Quoi/Pourquoi/Comment → Task 5
- ✅ 3 variantes avec matrices → Task 6
- ✅ Tableau technologies → Task 6
- ✅ Architecture + séquences + BDD → Task 7
- ✅ Mockups + stratégie de tests → Task 8
- ✅ Structure + Biome + Design → Task 9
- ✅ Stagiaires + Ateliers + Catégories → Task 10
- ✅ Certificat + BDD + Erreurs → Task 11
- ✅ Protocole de tests (tableau) → Task 12
- ✅ Problèmes rencontrés (guidé) → Task 12
- ✅ Améliorations → Task 12
- ✅ Glossaire → Task 12
- ✅ Placeholders candidat (Organisation, Gantt, Conclusion, Remerciements, Annexes) → Tasks 2 + 12

**2. Scan placeholders :** Tous les "À COMPLÉTER" sont explicitement balisés `> ⚠️`. Aucun TBD implicite.

**3. Cohérence :** Les noms de routes, tables, composants et services utilisés dans la section 6 correspondent exactement à ceux du code (`internshipRoutes.js`, `schema-v2.sql`).
